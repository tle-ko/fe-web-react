...
export default function MyCrewHistoryContainer() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          // 타임라인의 시작과 끝을 설정
          const start = new Date(Math.min(...response.data.map(crew => crew.latest_activity.date_start_at ? new Date(crew.latest_activity.date_start_at) : new Date())));
          const end = new Date(Math.max(...response.data.map(crew => crew.latest_activity.date_end_at ? new Date(crew.latest_activity.date_end_at) : new Date())));
          setTimelineStart(start);
          setTimelineEnd(addDays(end, 30)); // 끝 날짜에 30일을 더해 여유를 둡니다

          // 각 크루의 통계를 가져옵니다.
          const statisticsPromises = response.data.map(crew => 
            client.get(`api/v1/crew/${crew.id}/statistics`, {
              withCredentials: true
            }).then(statResponse => ({
              crewId: crew.id,
  ...
  return (
    <div className='box col-span-full w-full'>
      <p className='font-cafe24 text-2xl text-gray-900 mb-4'>나의 참여 크루</p>
        ...
        {isLoading ? (
          ...
          <>
            ...
            {crewData.map((crew) => {
              ...
              const width = crew.latest_activity.date_start_at && crew.latest_activity.date_end_at ? `${endPosition - startPosition}%` : '0%';
              
              return (
                <div key={crew.id} className="flex items-center mt-8">
                  <div className="p-4 bg-gray-100 rounded-xl"
                      style={{ width, marginLeft: `${startPosition}%` }}>
                    <div className="flex justify-start items-center gap-2 mb-1 text-gray-900 text-base font-medium">
                      <span>{crew.icon}</span>
                      <span className="whitespace-nowrap">{crew.name}</span>
                      <span>·</span>
                      <span className="whitespace-nowrap">{crew.latest_activity.name}</span>
                      <span>·</span>
                      <span className="whitespace-nowrap">{crewStatistics[crew.id] || 0}문제 풀이</span>
                    </div>
                  </div>
                </div>
              );
