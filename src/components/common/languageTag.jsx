...
const LanguageTag = ({ language, className = '' }) => {
  ...
  return (
    <div
      className={`tag min-w-fit ${languageColor()} flex items-center justify-center whitespace-nowrap rounded-[20px] px-3 py-2`}
    >
      {languageIcons[language] && (
        <img src={languageIcons[language]} alt={`${language} icon`} className="mr-1 h-4 w-4" />
      )}
      <p className="whitespace-nowrap text-sm font-semibold">{language}</p>
    </div>
  );
};
